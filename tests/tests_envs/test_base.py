...
class TestJobs:
    ...
    @pytest.mark.parametrize("idx, expected_result", [(0, (1, 0.5, 0)), (1, (2, 0.6, 0))])
    def test_get_item(self, sample_jobs, idx, expected_result):
        assert sample_jobs[idx] == expected_result

    @pytest.mark.parametrize("idx, status", [(0, JobStatus.RUNNING), (1, JobStatus.COMPLETE)])
...
def sample_cluster_object():
    ...
    usage = np.array([[2 for _ in range(10)], [3 for _ in range(10)], [4 for _ in range(10)], [5 for _ in range(10)],[6 for _ in range(10)]], dtype=np.float64)
