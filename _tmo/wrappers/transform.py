...
class ConcatenateObservationDict(gym.ObservationWrapper):
    ...
    def observation(self, observation: ObsType) -> WrapperObsType:
        ...
        # mapp Jobs
        queue[status == JobStatus.RUNNING] = -1
        queue[status == JobStatus.COMPLETE] = -1
        queue[status == JobStatus.NONEXISTENT] = -1
        #
        free_space = np.pad(
            free_space,
            ((0, max_nj - free_space.shape[0]), (0, 0), (0, 0)),
            mode="constant",
        )
        queue = np.pad(
            queue, ((0, max_nj - queue.shape[0]), (0, 0), (0, 0)), mode="constant"
        )
        n_shape = (2, max_nj, queue.shape[1], queue.shape[2])
        c_obs: np.ndarray = np.concatenate((free_space, queue), axis=0).reshape(n_shape)
        return c_obs
