...
class SimpleANNetwork(nn.Module):
    ...
    def __init__(self, state_shape: dict[str, gym.spaces.Box], n_action: int, n: int) -> None:
        super().__init__()
    ...
    def forward(self, obs, state=None, info={}):
        ...
        nodes = torch.tensor(nodes, dtype=torch.float) if not isinstance(obs, torch.Tensor) else nodes
        jobs = torch.tensor(jobs, dtype=torch.float) if not isinstance(obs, torch.Tensor) else jobs
...
class Net(nn.Module):
    def __init__(self, state_shape: dict[str, gym.spaces.Box], n_action: int, n: int):
        ...
        self.nodes_net = nn.Sequential(
            nn.Conv2d(in_channels=self.n, out_channels=16, kernel_size=(self.r, self.t)),
            nn.ReLU(),
        ...
        self.jobs_net = nn.Sequential(
            nn.Conv2d(in_channels=self.j, out_channels=16, kernel_size=(self.r, self.t)),
            nn.ReLU(),
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=1),
            nn.ReLU(),
            nn.Flatten(1)
...
def create_env(render_mode='',n_jobs: int = 5, n_nodes: int = N_NODES):
    return gym.make(
        "cluster-v0", nodes=n_nodes, jobs=n_jobs, resource=1, time=2, render_mode=render_mode
    )
