...
class Jobs:
    ...
    arrival: npt.NDArray[np.uint32]  # time jobs arrive to the cluster [1D]
    usage: npt.NDArray[np.float64]  # usage of job throw time [2D]
    _status: npt.NDArray[np.uint32] = field(init=False)  # job status in cluster [1D]
    _length: npt.NDArray[np.uint32] = field(init=False)  # length of each job [1D]
...
class ClusterObject:
    ...
    jobs: Jobs  #
    _empty_job_cell_val: int = field(init=False, default=-1)  # how to fill empty cells
    _run_time: npt.NDArray[np.uint32] = field(init=False)  # run time options
    _usage: npt.NDArray[np.float64] = field(init=False)  # usage uptions
    _time: int = field(init=False, default=0)  # start cluster tick
    _logger: logging.Logger = field(init=False)  # logger
    ...
    @property
    def n_jobs(self) -> int:
        """Jobs in either state in the cluster."""
    ...
    def __post_init__(self):
        ...
        self._logger.debug(
            f"Created cluster with; nodes: {self.n_nodes}, jobs: {self.n_jobs}"
        )
    ...
    def tick(self):
        ...
        self._time += 1
        # create index
        iteration_complete_jobs: npt.NDArray[np.bool_] = (
            self._run_time == self.jobs.length
        )
        arrived_jobs: npt.NDArray[np.bool_] = self.jobs.arrival == self._time
        self._logger.info(f"After tick time: {self._time}")
        # logging
        self._logger.info(
            f"Iteration completed jobs: {np.where(iteration_complete_jobs)}"
        )
    ...
    def schedule(self, n_idx: int, j_idx: int) -> bool:
        ...
        logging.info(
            f"Try to schedule job {j_idx} with status {JobStatus(job_status).name}"
        )
        match job_status:
            case JobStatus.PENDING:
                job_can_be_schedule: bool = False
                node_free_space: npt.NDArray[np.float64] = (
                    self.nodes[n_idx] - self._usage[n_idx]
                )
